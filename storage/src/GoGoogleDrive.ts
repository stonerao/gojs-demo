...
/**
 * <p>Class for saving / loading GoJS <a href="https://gojs.net/latest/api/symbols/Diagram.html">Diagram</a> <a href="https://gojs.net/latest/api/symbols/Model.html">
 * models</a> to / from Google Drive. Uses the <a href="https://developers.google.com/drive/v3/reference/">Google Drive V3 API</a> by use of a
 ...
 * <p><b>Note</b>: Any page using GoGoogleDrive must include a script tag with src set to https://apis.google.com/js/api.js. </p>
 * @category Storage
 */
export class GoGoogleDrive extends gcs.GoCloudStorage {
    ...
    /**
     * <p>Get Google API Client. The Google API Client is used in GoGoogleDrive to make many different requests to Google Drive, however, it
     ...
     * authorization in {@link authorize}.</p>
     * <p>gapiClient is really of type Object, not type any. However, the Google libraries are all written in JavaScript and do not provide
     * d.ts files. As such, to avoid TypeScript compilation errors, both gapiClient and {@link gapiPicker} properties are declared as type any.</p>
     * @function.
     * @return {any}
     */
     ...
     * <p>gapiPicker is really of type Object, not type any. However, the Google libraries are all written in JavaScript and do not
     * provide d.ts files. As such, to avoid TypeScript compilation errors, both {@link gapiClient} and gapiPicker properties are declared as type any.</p>
     * @function.
     * @return {any}
     */
    ...
    public createPicker(cb: Function) {
        ...
        if (storage._oauthToken) {
            ...
            let appId = storage.clientId.substring(0, this.clientId.indexOf("-"));
    ...
    public getUserInfo() {
        const storage = this;
        return new Promise(function (resolve: Function, reject: Function) {
            let request = storage.gapiClient.request({
                'path': '/drive/v3/about',
                'method': 'GET',
                'params': { "fields": "user" },
    ...
    /**
     * <p>Get the Google Drive file reference object at a given path. Fields include: </p>
     * <ul>
     *  <li>id: The Google Drive-given ID of the file at the provided path</li>
     *  <li>name: The name of the file saved to Google Drive at the provided path</li>
     *  <li>mimeType: For diagram files, this will always be <code>text/plain</code></li>
     *  <li>kind: This will usually be <code>drive#file</code>.</li>
     * </ul>
     * <p><b>Note:</b> Name, ID, and path values are requisite for creating valid {@link DiagramFile}s. When creating a DiagramFile for a
     * diagram saved to Google Drive, provide the same value for name and path properties.</p>
    ...
    public getFile(path: string) {
        const storage = this;
        return new Promise(function (resolve: Function, reject: Function) {
            let req = storage.gapiClient.request({
    ...
    public saveWithUI() {
        ...
        return new Promise(function (resolve: Function, reject: Function) {
            if (ui.style.visibility === 'hidden') {
                ...
                let saveName: string = (<HTMLInputElement>document.getElementById('userInput')).value;
        ...
        return new Promise(function (resolve: Function, reject: Function) {
            if (path) { // save as
                ...
                let request = storage.gapiClient.request({
                    ...
                    callback: function (resp) {
                        let savedDiagrams: Object[] = resp.files;
                        if (savedDiagrams) {
                        ...
                        let data = storage.makeSaveFile();

                        let multipartRequestBody: string =
                            delimiter +
                        ...
                        request.execute(function (resp) {
                            let savedFile: gcs.DiagramFile = { name: resp.name, id: resp.id, path: resp.name };
                ...
                let fileId: string = storage.currentDiagramFile.id;
                let saveFile: string = storage.makeSaveFile();
    ...
    public loadWithUI() {
        const storage = this;
        return new Promise(function (resolve: Function, reject: Function) {
            let loadFunction: Function = function (data) {
                    ...
                    callback: function (modelData) {
                        if (modelData) {
                            if (file.name.indexOf(".diagram") !== -1) {
                                ...
                                let loadedFile: gcs.DiagramFile = { name: file['name'], path: file['name'], id: file['id'] };
                                storage.currentDiagramFile = loadedFile;
                                resolve(loadedFile);
                            }
                        }
    ...
    public removeWithUI() {
        const storage = this;
        return new Promise(function (resolve: Function, reject: Function) {
            var deleteFunction = function (data: Object) {
                if (data['action'] === "picked") {
                    var file = data['docs'][0];
