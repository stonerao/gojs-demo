 ...
 * to / from Microsoft One Drive.
 * As with all {@link GoCloudStorage} subclasses (with the exception of {@link GoLocalStorage}, any page using GoDropBox must be served on a web server.</p>
 * <p><b>Note</b>: Makes use of <a href="https://dev.onedrive.com/sdk/js-v72/js-picker-overview.htm">OneDrive FilePicker for JavaScript v7.2</a>. Any page
 * using GoOneDrive must include a script tag with src set to https://js.live.net/v7.2/OneDrive.js. </p>
 * @category Storage
 */
export class GoOneDrive extends gcs.GoCloudStorage {
    ...
    /**
    * @constructor
    * @param {go.Diagram[]} managedDiagrams An array of GoJS <a href="https://gojs.net/latest/api/symbols/Diagram.html">Diagrams</a> whose model(s) will be saved to
    ...
    * will be empty. Usually a value given by calling <a href="https://gojs.net/latest/api/symbols/Model.html#toJson">.toJson()</a> on a GoJS Diagram's Model.
    ...
    */
     ...
     */
    public authorize(refreshToken?: boolean) {
        const storage = this;
        return new Promise(function(resolve: Function, reject: Function){
            if (!refreshToken && window.location.hash.indexOf("access_token") !== -1) {
                ...
                let authUrl: string = 'https://login.microsoftonline.com/common/oauth2/v2.0/authorize?client_id=' + storage.clientId +
     ...
     * </ul>
     * <p><b>Note:</b> If {@link oauthToken} is not valid or has expired, a page redirect to the Microsoft Account sign in will occur.</p>
     * @return {Promise} Returns a Promise that resolves with information about the currently logged in Microsoft user
     */
    public getUserInfo() {
        const storage = this;
        return new Promise(function(resolve: Function, reject: Function) {
     ...
     *  </li>
     * <ul>
     * <p><b>Note:</b> Name, ID, and path are requisite for creating valid {@link DiagramFile}s. A path can be constructed by concatenating the
     ...
     */
    public getFile(path: string, token?: string) {
        const storage = this;
        if (path.indexOf('.diagram') === -1) path += '.diagram';
        return new Promise(function (resolve: Function, reject: Function) {
            let xhr: XMLHttpRequest = new XMLHttpRequest();
            xhr.open('GET', 'https://graph.microsoft.com/v1.0' + path, true);
            let t: string = (token) ? token : storage.oauthToken;
            xhr.setRequestHeader('Authorization', 'Bearer ' + t);
            xhr.onreadystatechange = function () {
                if (xhr.readyState == 4) {
                    if (xhr.status == 200) {  // 200=OK
                        let file: Object = JSON.parse(xhr.response);
                        resolve(file);
    ...
    public saveWithUI() {
        ...
        return new Promise(function (resolve, reject) {
            if (ui.style.visibility === 'hidden') {
                ...
                let saveName = (<HTMLInputElement>document.getElementById('userInput')).value;
                if (saveName && saveName.indexOf('.diagram') === -1) saveName += ".diagram";
                let odOptions: Object = {
                    clientId: storage.clientId,
                    action: "query",
                    openInNewWindow: true,
                    success: function (selection) {
                        let folder: Object = selection.value[0];
                        let token: string = selection.accessToken;
     ...
     * <code>/drive/root:/{path-to-file}/{filename}</code>; i.e. <code>/drive/root:/Documents/example.diagram</code>
     * @return {Promise<any>} Returns a Promise that resolves with a {@link DiagramFile} representing the saved file.
     */
    public save(path?: string) {
        ...
        return new Promise(function (resolve: Function, reject: Function) {
            if (path) { // save as
                let xhr: XMLHttpRequest = new XMLHttpRequest();
                if (path.indexOf(".diagram") === -1) path += '.diagram';
                let bodyContent: string = storage.makeSaveFile();
                ...
                xhr.onreadystatechange = function () {
                    if (xhr.readyState == 4) {
                        if (xhr.status >= 200 && xhr.status < 300) {
                            let file: Object = JSON.parse(xhr.response);
                            let savedFile: gcs.DiagramFile = { name: file['name'], id: file['id'],
                ...
                let token: string = storage.currentDiagramFile.token;
                let url: string = storage.generateGraphUrl(storage.currentDiagramFile, true, true);
                let bodyContent: string = storage.makeSaveFile();

                let t: string = (!token) ? storage.oauthToken : storage.currentDiagramFile.token;
                let xhr: XMLHttpRequest = new XMLHttpRequest();
        ...
        return new Promise(function (resolve: Function, reject: Function) {
            let odOptions: Object = {
                ...
                success: function (files: Object) {
                    let file: Object = files['value'][0];
                    let token: string = files['accessToken'];
                    let filePath = file['parentReference']['path'] + "/" + file['name'];
     ...
     * @return {Promise<any>} Returns a Promise that resolves with a {@link DiagramFile} representing the loaded file
     */
    public load(path: string, token?: string) {
        const storage = this;
        return new Promise(function (resolve: Function, reject: Function) {
            if (path) {
                let t: string = (token) ? token : storage.oauthToken;
                storage.getFile(path, t).then(function (file: Object) {
                    let downloadLink: string = file["@microsoft.graph.downloadUrl"];
                    // Download file from download link
                    let downloadxhr: XMLHttpRequest = new XMLHttpRequest();
                    ...
                    downloadxhr.onreadystatechange = function () {
                        if (downloadxhr.readyState == 4) {
                            if (downloadxhr.status == 200) {
                                ...
                                let loadedFile: gcs.DiagramFile = { name: file['name'], id: file['id'], path: file['parentReference']['path'] + '/' + file['name'], token: token,
            ...
            var odOptions = {
                ...
                success: function (files: Object) {
                    if (files) {
                        let file: Object = files['value'][0];
                        let token: string = files['accessToken'];
                        let filePath: string = file['parentReference']['path'] + "/" + file['name'];
                        resolve(new Promise(function (resolve: Function, reject: Function) {
                            resolve(storage.remove(filePath, token));
                        }));
                    }
                }
            };
     ...
     * @return {Promise<any>} Returns a Promise that resolves with a {@link DiagramFile} representing the deleted file
     */
    public remove(path: string, token?: string) {
        const storage = this;
        let t: string = (token) ? token : storage.oauthToken;
        return new Promise(function (resolve: Function, reject: Function) {
            storage.getFile(path, t).then(function (file: Object) {
                let deletedFile: gcs.DiagramFile = { name: file['name'], id: file['id'], path: file['parentReference']['path'] + '/' + file['name'] };
                let xhr: XMLHttpRequest = new XMLHttpRequest();
