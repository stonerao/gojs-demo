 ...
 * <p>DiagramFiles are used as a minimal representation of files stored in data. {@link GoCloudStorage.currentDiagramFile}.
 ...
 * DiagramFiles containing minimal data about the saved / loaded / removed / created file in storage. </p>
 * @category Storage
 */
export interface DiagramFile {
    /**
     * The storage-given ID of the diagram file. This is usually a lengthy alphanumeric string.
     * <p><b>Note</b>: In the case of files saved to / loaded from Local Storage with {@link GoLocalStorage},ID is the same as the name of the file.
     */
    id: string;
    /**
     ...
     * <ul>
     *  <li>{@link GoLocalStorage}: Just the filename (the key in local storage); i.e. <code>example.diagram</code></li>
     *  <li>{@link GoDropBox}: <code>/{path-to-file}/{filename}</code>; i.e. <code>/Public/example.diagram</code></li>
     *  <li>{@link GoGoogleDrive}: Use Google Drive-given file IDs. Parameter is still called 'path' in GoGoogleDrive methods to preserve system nomenclature.</li>
     *  <li>{@link GoOneDrive}: <code>/drive/root:/{path-to-file}/{filename}</code>; i.e. <code>/drive/root:/Documents/example.diagram</code></li>
     * </ul>
     */
    path: string;
    /**
     * @private
     * @hidden
     * token is sometimes necesary for {@link GoOneDrive}. This is an access token given by the <a href="https://dev.onedrive.com/sdk/js-v72/js-picker-overview.htm">
 ...
 * You may call <code>.resolve</code> and <code>.reject</code> on this field as you may a standard ES6 Promise. After resolving / rejecting
 * deferredPromise.promise, it is recommended you reset it by calling {@link GoCloudStorage.makeDeferredPromise}. Example:</p>
 *
 * ```js
 *
...
/**
 * An abstract class for storing GoJS <a href="https://gojs.net/latest/api/symbols/Diagram.html">Diagram</a>
 * <a href="https://gojs.net/latest/api/symbols/Model.html">models</a> in various cloud storage services.
 * <p>GoCloudStorage is never used on its own. Its subclasses can be used to manage diagram model storage programatically,
 * or any subset of GoCloudStorage subclasses can be bundled and used graphically with the {@link GoCloudStorageManager}.</p>
 * <p><strong>Note</strong>: With the exception of {@link GoLocalStorage}, all GoCloudStorage subclasses must be used in pages served
 * on a web server.</p>
 * @category Storage
 */
export class GoCloudStorage {
     ...
     * Usually a value given by calling <a href="https://gojs.net/latest/api/symbols/Model.html#toJson">.toJson()</a> on a GoJS Diagram's Model.
    ...
    */
    ...
    /**
     * Get / set the GoJS <a href="https://gojs.net/latest/api/symbols/Diagram.html">Diagram</a>s associated with this instance of GoCloudStorage.
    ...
    */
     ...
     * <a href="https://gojs.net/latest/api/symbols/Transaction.html"> Transaction</a> (only if {@link currentDiagramFile} holds a non-null path value).
     * Additionally, if isAutoSaving is true, users will be prompted to save newly created diagrams when created with {@link create}.
     * The default value for isAutoSaving is <code>true</code>.
     * @function.
     * @return {boolean}
    */
    ...
    */
    ...
    /**
     * <p>Get / set deferredPromise property. This is a special property that is simply an Object with a "promise" field, which contains an
     * ES6 <a href="http://es6-features.org/#PromiseUsage">Promise</a>. DeferredPromise is important in that it allows for a Promise to be
     * returned by one function and resolved within another. You may call <code>.resolve</code> and <code>.reject</code> on this field as you may
     * a standard ES6 Promise. After resolving / rejecting deferredPromise.promise, it is recommended you reset it by calling {@link makeDeferredPromise}. Example:</p>
     *
     * ```js
     *
     * // function a returns the "promise" field of deferredPromise
    ...
    *
     ...
     * instance of GoCloudStorage (via {@link clientId}. If no currently signed-in user exists, prompt user to sign into their account, then authorize that account.</p>
     * <p><b>Note:</b> Authorization does not occur (and is not possible or necessary) with {@link GoLocalStorage}. Instead,
     * {@link GoLocalStorage.authorize} ensures localStorage exists in the browser.</p>
     ...
     * <ul>
     *  <li>{@link GoLocalStorage.authorize}</li>
     *  <li>{@link GoDropBox.authorize}</li>
     *  <li>{@link GoGoogleDrive.authorize}</li>
     *  <li>{@link GoOneDrive.authorize}</li>
     * </ul>
    ...
    */
    public makeDeferredPromise(): Promise<Object> {
        ...
        let promise: any = new Promise((resolve: Function, reject: Function) => {
            res = resolve;
            rej = reject;
        });
        promise.resolve = res;
     ...
     * <ul>
     *  <li>{@link GoLocalStorage.getUserInfo}</li>
     *  <li>{@link GoDropBox.getUserInfo}</li>
     *  <li>{@link GoGoogleDrive.getUserInfo}</li>
     *  <li>{@link GoOneDrive.getUserInfo}</li>
     * </ul>
    ...
    */
     ...
     * <ul>
     *  <li>{@link GoLocalStorage.getFile}</li>
     *  <li>{@link GoDropBox.getFile}</li>
     *  <li>{@link GoGoogleDrive.getFile}</li>
     *  <li>{@link GoOneDrive.getFile}</li>
     * </ul>
     * @param {string} path A valid filepath. What is meant by this varies from subclass to subclass. Rules for valid filepaths by subclass:
     * <ul>
     *  <li>{@link GoLocalStorage}: Just the filename (the key in local storage); i.e. <code>example.diagram</code></li>
    ...
    */
     ...
     * each model will be set to a new <a href="https://gojs.net/latest/api/symbols/GraphLinksModel.html">GraphLinksModel</a> by default).
     ...
     * @param {string} path If a valid filepath is supplied, save each of {@link managedDiagrams}' model data to this path. No UI of any sort appears.
     ...
     *  <li>{@link GoOneDrive}: <code>/drive/root:/{path-to-file}/{filename}</code>; i.e. <code>/drive/root:/Documents/example.diagram</code></li>
     * </ul>
     * @return {boolean} Returns a Promise that resolves with a {@link DiagramFile} representing the newly created file.
    */
    public create(path?: string) {
        ...
        return new Promise(function (resolve: Function, reject: Function) {
            ...
            } else for (let i = 0; i < storage.managedDiagrams.length; i++) {
    ...
    protected makeSaveFile() {
        ...
        for (let i = 0; i < storage.managedDiagrams.length; i++) {
            let diagram: go.Diagram = storage.managedDiagrams[i];
            let div: string = diagram.div.id;
    ...
    protected loadFromFileContents(fileContents: string) {
        const storage = this;
        let models = JSON.parse(fileContents);
        for (let divId in models) {
            let model = models[divId];
            let div: HTMLDivElement = <HTMLDivElement>(document.getElementById(divId));
            let diagram: go.Diagram = go.Diagram.fromDiv(div);
            if (diagram) {
