...
export class GoNetCore extends gcs.GoCloudStorage {
    ...
    public showUI(action: string, numAdditionalFiles?: number) {
        ...
        let maxFilesToShow: number = GoNetCore._MIN_FILES_IN_UI + numAdditionalFiles;
        ...
        let numFilesToCheck: number = GoNetCore._MIN_FILES_IN_UI + numAdditionalFiles;
        ...
        storage.getFiles().then(function(files: Array<any>) {
            if (files.length !== 0) {
                for (let i in files) {
                    ...
                    if (savedDiagrams.length < maxFilesToShow) {
                        numFilesChecked++;
                        let fileContent: string = item.file;
                        if (fileContent && fileContent.indexOf("GraphLinksModel" || "TreeModel") !== -1) {
                            let file: Object = { name: item.name, id: item.id };
            ...
            if (storage.currentDiagramFile.id) {
                let el: HTMLElement = document.getElementById(storage.currentDiagramFile.id + '-label');
    ...
    /**
     * <p>Get information about a diagram file saved to .NET Core database. This data includes:</p>
     * <ul>
    ...
    public getFile(path: string) {
        const storage = this;
        let url: string = storage.rootEndpoint + path;
        ...
        return new Promise(function (resolve: Function, reject: Function) {
            ...
            if (storage.currentDiagramFile.id && !path) {
                ...
                storage.getFile(storage.currentDiagramFile.id).then(function(resp){
                    ...
                    let savedFile: gcs.DiagramFile = {id: item.id, name: item.name, path: item.name};
            ...
            // POST -- create a new entry at the provided name (path)
            else {
                let xhr: XMLHttpRequest = new XMLHttpRequest();
                let item = {
                ...
                xhr.onreadystatechange = function () {
                    if (xhr.readyState == 4) {
                        if (xhr.status > 200 && xhr.status < 300) {
                            ...
                            let savedFile: gcs.DiagramFile = {id: id, name: item.name, path: item.name};
                ...
                xhr.onreadystatechange = function () {
                    if (xhr.readyState == 4) {
                        if (xhr.status >= 200 && xhr.status < 300) {
                            ...
                            let loadedFile: gcs.DiagramFile = { id: respJSON.id, path: respJSON.name, name: respJSON.name };
        ...
        return new Promise(function (resolve: Function, reject: Function) {
            if (path) {
                storage.getFile(path).then(function(resp: any){
                    let deletedFile: gcs.DiagramFile = { name: resp.name, path: resp.name, id: path };
